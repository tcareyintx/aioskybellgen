...
class SkybellDevice:
    ...
    async def _async_create_trggers_request(
        self, json: dict[str, str | list[str]]
    ) -> str | None:
    ...
    async def async_create_trigger(
        self, body: dict[str, str | list[str] | None]
    ) -> str:
        ...
        body dict:
            ...
            webhook_header: Header name for the the webhook.
            header_value: Header value for the associated webhook header.
        ...
        """
        # Unpack the body
        webhook_url: str = str(body.get("webhook_url"))
        webhook_method: str = str(body.get("webhook_method"))
        events: list[str] = cast(list, body.get("events"))
        webhook_header: str = str(body.get("webhook_header"))
        header_value: str = str(body.get("header_value"))

        _LOGGER.debug("Setting a trigger to : %s for %s", webhook_url, self.device_id)
    ...
    async def async_delete_trigger(self, trigger_id: str) -> None:
        ...
        created.
        """
        _LOGGER.debug("Deleting a trigger: %s for %s", trigger_id, self.device_id)
